...
export function useCanonicalEquipmentFilters(categoryId: string, canReadProjects = true) {
  ...
  useEffect(() => {
    ...
    void Promise.all([readRepositories.equipmentCategories.list(), repositories.equipmentStatusRead.list(), projectResult]).then(([categoryResult, statusResult, projectsResult]) => {
      if (!active) return;
    ...
    return () => { active = false; };
  }, [canReadProjects, readRepositories.equipmentCategories, readRepositories.projects, repositories.equipmentStatusRead]);
  ...
  return { categories, subcategories, statuses, projects };
}
