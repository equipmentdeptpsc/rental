...
const primary = new Set(["operation", "idle", "mealBreak", "breakdown"]);
export function deriveDeurEventState(record: Pick<DeurRecord, "events" | "status" | "legacy">) {
  ...
  const openPrimaryActivity = [...open.keys()].find((x) => primary.has(x)) as "operation" | "idle" | "mealBreak" | "breakdown" | undefined;
