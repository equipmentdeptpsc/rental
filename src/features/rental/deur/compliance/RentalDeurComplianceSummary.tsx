...
export default function RentalDeurComplianceSummary({ result, policy, policyStaged = false }: { result: RentalDeurComplianceResult; policy?: RentalDeurExpectationPolicy; policyStaged?: boolean }) {
  return <>
    ...
    {result.expectations.length > 0 && <div className="mt-4 overflow-x-auto border-t pt-4"><table className="min-w-full text-xs">
      <thead><tr>{["Work Date", "Shift", "Window", "Due State", "Compliance Status", "DEUR", "Reason"].map((heading) => <th key={heading} className="px-2 py-1 text-left">{heading}</th>)}</tr></thead>
      <tbody>{result.expectations.map((expectation) => <tr key={expectation.expectationId} className="border-t"><td className="px-2 py-1">{expectation.workDate}</td><td className="px-2 py-1">{expectation.shiftCode ?? "—"}</td><td className="px-2 py-1">{expectation.startTime && expectation.endTime ? `${expectation.startTime}–${expectation.endTime}${expectation.crossesMidnight ? " next day" : ""}` : "—"}</td><td className="px-2 py-1">{expectation.expectationStatus.replaceAll("_", " ")}</td><td className="px-2 py-1">{expectation.status.replaceAll("_", " ")}</td><td className="px-2 py-1">{expectation.matchingDeurNumber ?? expectation.matchingEffectiveDeurId ?? "—"}{expectation.matchingRevisionNumber ? ` R${expectation.matchingRevisionNumber}` : ""}</td><td className="px-2 py-1">{expectation.reason}</td></tr>)}</tbody>
    </table></div>}
  </>;
}
