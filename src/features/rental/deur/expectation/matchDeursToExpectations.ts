...
export function matchDeursToExpectations({ expectations, deurs }: { expectations: DeurExpectation[]; deurs: DeurRecord[] }) {
  ...
  const results = expectations.map<RentalDeurExpectationResult>((expectation) => {
    ...
    if (expectation.shiftCode && expectation.status !== "DUE") return { ...base, status: expectation.status, reason: expectation.status === "CURRENT" ? "Shift is still in progress." : "Shift has not started.", ...(incomplete ? { matchingEffectiveDeurId: incomplete.id, matchingDeurNumber: incomplete.deurNumber, matchingRevisionNumber: incomplete.revision?.revisionNumber ?? 1 } : {}) };
    if (incomplete) return { ...base, status: "INCOMPLETE", reason: "Shift completed; DEUR is awaiting acknowledgement.", matchingEffectiveDeurId: incomplete.id, matchingDeurNumber: incomplete.deurNumber, matchingRevisionNumber: incomplete.revision?.revisionNumber ?? 1 };
