import { normalizeRentalBillingMethod, type RentalCommercialSnapshot } from "../types";
...
export function createRentalCommercialSnapshot(source:Source,capturedAt:string):CreateRentalCommercialSnapshotResult{
 ...
 const billingMethod=normalizeRentalBillingMethod(source.billingMethod);
 if(!billingMethod)issues.push({code:"INVALID_BILLING_METHOD",message:"A supported billing method is required.",field:"billingMethod"});
