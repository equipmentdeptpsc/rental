...
export default function RemoteOperatorCertificationEditor() {
  ...
  const [historical, setHistorical] = useState<OperatorCertificationAssignment[]>([]);
  const [selected, setSelected] = useState<string[]>([]);
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);
  const refreshAssignments = useCallback(async () => { if (!id) return false; const current = await readRepositories.operatorCertifications.listForOperator(id); if (!current.success) { setMessage("Certification assignments were saved, but the canonical refresh failed. Retry safely."); return false; } setHistorical(current.value); setSelected(current.value.filter((item) => item.active).map((item) => item.certificationTypeId)); return true; }, [id, readRepositories.operatorCertifications]);
