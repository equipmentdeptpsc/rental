...
export class SupabaseCanonicalRentalRepository implements CanonicalRentalRemoteRepository {
  ...
  async readWorkspace(rentalId: string) { return this.read<CanonicalRentalWorkspace>("read_canonical_rental_workspace", { target_rental_id: rentalId }, value => ({ rentalId: String(value.rentalId), contracts: array(value.contracts), commercialSnapshots: array(value.commercialSnapshots) })); }
