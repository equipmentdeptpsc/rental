...
function ResponsiveEquipmentTable({ items }: { items: ReturnType<typeof useCanonicalEquipmentData>["items"] }) {
  return <div className="app-card overflow-x-auto"><table className="app-table min-w-full table-fixed"><thead><tr><th className="w-[18%] p-3 text-left">Asset No.</th><th className="w-[34%] p-3 text-left">Equipment</th><th className="w-[20%] p-3 text-left">Category</th><th className="w-[16%] p-3 text-left">Status</th><th className="w-[12%] p-3 text-right">Action</th></tr></thead><tbody>{items.map((item) => <tr className="odd:bg-slate-50/40 hover:bg-amber-50/60 dark:odd:bg-slate-800/20 dark:hover:bg-amber-950/20" key={item.id}><td className="p-3 align-middle">{item.assetNo}</td><td className="p-3 align-middle">{item.equipmentName}</td><td className="p-3 align-middle">{item.category ?? "—"}</td><td className="p-3 align-middle"><StatusBadge className={statusBadgeClass(item.statusLabel)} tone={statusBadgeTone(item.statusLabel)}>{item.statusLabel ?? "Unavailable"}</StatusBadge></td><td className="p-3 text-right align-middle"><Link aria-label={`View ${item.equipmentName}`} className="inline-flex rounded p-1 text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-800" to={`/equipment/${item.id}`}><ChevronRight size={16} aria-hidden="true" /></Link></td></tr>)}</tbody></table></div>;
}
