...
function ResponsiveEquipmentTable({ items }: { items: ReturnType<typeof useCanonicalEquipmentData>["items"] }) {
  return <div className="app-card overflow-x-auto"><table className="app-table min-w-full"><thead><tr><th>Asset No.</th><th>Equipment</th><th className="w-28">Category</th><th>Status</th><th className="text-right">Action</th></tr></thead><tbody>{items.map((item) => <tr className="odd:bg-slate-50/40 hover:bg-amber-50/60 dark:odd:bg-slate-800/20 dark:hover:bg-amber-950/20" key={item.id}><td>{item.assetNo}</td><td>{item.equipmentName}</td><td>{item.category ?? "—"}</td><td><StatusBadge className={statusBadgeClass(item.statusLabel)} tone={statusBadgeTone(item.statusLabel)}>{item.statusLabel ?? "Unavailable"}</StatusBadge></td><td className="text-right"><Link aria-label={`View ${item.equipmentName}`} className="inline-flex rounded p-1 text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-800" to={`/equipment/${item.id}`}><ChevronRight size={16} aria-hidden="true" /></Link></td></tr>)}</tbody></table></div>;
}
