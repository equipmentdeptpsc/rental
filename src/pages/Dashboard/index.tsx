...
function Legend({ label, value, color }: { label: string; value: number; color: string }) { return <span className="inline-flex items-center gap-1.5 text-slate-600 dark:text-slate-300"><span aria-hidden="true" className={`h-2 w-2 rounded-sm ${color}`} />{label}<strong>{value}</strong></span>; }
