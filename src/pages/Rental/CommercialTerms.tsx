...
export default function RentalCommercialTermsPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6 p-6">
    ...
    <section className="rounded-xl border bg-white p-4"><h2 className="font-semibold">Equipment Lines</h2><div className="mt-3 grid gap-3 sm:grid-cols-2">{lines.map((line) => { const machine = equipment.find((item) => item.id === line.equipmentId); const contract=getContractForRentalEquipmentLine(line.id); const effective=line.commercialSnapshot??contract; const state=line.commercialSnapshot?"FROZEN":contract?"CONFIGURED":"REQUIRED"; return <button type="button" key={line.id} onClick={() => setSelectedId(line.id)} className={`min-w-0 rounded-lg border p-4 text-left ${selected?.id === line.id ? "border-blue-500 bg-blue-50" : state==="REQUIRED"?"border-amber-400 bg-amber-50":state==="FROZEN"?"bg-slate-50 text-slate-700":""}`}><strong className="block truncate">{machine ? `${machine.assetNo} - ${machine.equipmentName}` : line.equipmentId}</strong><span className={`mt-2 inline-block text-sm font-semibold ${state==="REQUIRED"?"text-amber-800":state==="CONFIGURED"?"text-green-700":"text-slate-700"}`}>{state==="REQUIRED"?"⚠ Commercial Terms Required":state==="CONFIGURED"?"✓ Configured":"🔒 Frozen"}</span>{effective&&<p className="mt-1 text-sm"><span className="block">Billing Method: {effective.billingMethod}</span><span className="block">Rate: {(effective.currency??"PHP")} {(effective.unitRate??effective.contractAmount??0).toLocaleString()} / {effective.billingMethod.replace("Per ","").toLowerCase()}</span></p>}<p className="mt-1 text-xs text-slate-500">Rental line status: {line.status}</p></button>; })}</div>
