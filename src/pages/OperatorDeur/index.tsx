...
export function mergeOperatorDeurVersions(current: Record<string, number>, deurs: readonly DeurRecord[]): Record<string, number> {
  const next = Object.fromEntries(deurs.map((record) => {
    const persisted = Number((record as DeurRecord & { rowVersion?: number }).rowVersion ?? 0);
    return [record.id, Math.max(current[record.id] ?? 0, persisted)];
  }));
...
export default function OperatorDeurPage() {
  const { rentalId = "" } = useParams(); const [searchParams,setSearchParams]=useSearchParams(); const { user } = useAuth(); const dependencies=useApplicationDependenciesCompatibility(); const { commandRepositories,changeNotifications,synchronization,authentication } = dependencies; const remote=dependencies.configuration.persistenceMode==="remote"; const data=useOperatorDeurData(rentalId,user); const { rental,lines:rentalEquipmentLines,assignments,operators,equipment,projects,deurs:persistedDeurs,workDescriptions,refresh }=data;
  ...
  const [, setVersion] = useState(0), [versions,setVersions]=useState<Record<string,number>>({}), [clock, setClock] = useState(() => new Date().toISOString()), [message, setMessage] = useState(""), [remarks, setRemarks] = useState(""), [pendingAction,setPendingAction]=useState<DeurOperatorAction | "SUBMIT">();
  ...
  const windowSnapshot = shift ? configuredWindows.find((item) => item.code === (shift === "Night" ? "NIGHT" : "DAY")) : undefined;
  ...
  const access = routeResolution.status !== "RESOLVED"
    ...
    : evaluateOperatorDigitalDeurAccess({ actor: user ?? undefined, authenticatedOperatorId: accountAccess.operator.id, operator: accountAccess.operator, assignment, rental, rentalEquipmentLine: selectedLine, deurs, evaluationTimestamp: clock, shift, serverAuthoritativeCommercialEvidence:remote });
  ...
  async function startDigitalDeur() {
    ...
    const prepared = prepareDeur({ authenticatedUser: user, enforceOperatorOwnership: true, serverAuthoritativeCommercialEvidence:remote, rentalId: rental.id, rentalEquipmentLineId: selectedLine.id, rentalEquipmentLine:selectedLine, rentalStatus: rental.status, rental, assignmentId: selectedLine.assignmentId, equipmentId: selectedLine.equipmentId, operatorId: selectedLine.operatorId, projectId: rental.projectId, customerId: rental.customerId, selectedWorkDescription: selected, remarks, shift, openingMeter: beginning, meterReadingType, existingDeurs:deurs, odometerCheckpoints: evidenceMode.supported && evidenceMode.mode === "ODOMETER_TRIP" ? [{ id: crypto.randomUUID(), location: startLocation, odometerReading: beginning!, recordedAt: new Date().toISOString() }] : undefined, completionEvidence: evidenceMode.supported && evidenceMode.mode === "COMPLETION" ? { status: "IN_PROGRESS" } : undefined });
  ...
  async function applyAction(action: DeurOperatorAction) {
    if (!active || !user || pendingAction) return; if (action === "END_SHIFT" && !window.confirm("End the current shift and close its active activity?")) return;
    ...
    try {
      ...
      else if(result.refreshRequired){if(result.currentVersion!==undefined)setVersions(current=>({...current,[active.id]:Math.max(current[active.id]??0,result.currentVersion!)}));await refresh();}
      setVersion((value) => value + 1);
    } finally { setPendingAction(undefined); }
  }
  async function submit() {
    if (!active || !user || pendingAction || !window.confirm("Submit this DEUR for acknowledgement? It will no longer be editable.")) return;
    setPendingAction("SUBMIT");
    try {
      const commandId=crypto.randomUUID(),input={commandId,idempotencyKey:commandId,rentalId:active.rentalId,rentalLineId:active.rentalEquipmentLineId??"",equipmentId:active.equipmentId,operatorId:active.operatorId,assignmentId:active.assignmentId??"",deurId:active.id,expectedVersion:versions[active.id]??0,clientCreatedAt:new Date().toISOString()};
      const gatewayResult=await commandGateway.executeOrQueue({type:"DEUR_SUBMIT",input},await currentReplayIdentity(input.operatorId,input.assignmentId));
      if(gatewayResult.disposition==="QUEUED"){applyOfflineProjection({type:"DEUR_SUBMIT",input});setMessage("DEUR submission saved offline and will synchronize after reconnection.");return;}
      const result=gatewayResult.result;
      setMessage(result.success ? "DEUR submitted. Waiting acknowledgement." : result.submissionIssues?.length?formatDeurSubmissionIssues(result.submissionIssues):result.message);
      if(result.success){if(synchronization.tenantId&&synchronization.publishEnabled)for(const event of createDeurOperationalEvents({tenantId:synchronization.tenantId,deur:result.record,action:"SUBMIT",serverOccurredAt:result.serverOccurredAt,aggregateVersion:result.version}))await synchronization.operator.publish(event);setVersions(current=>({...current,[result.record.id]:result.version}));await refresh();}
      setVersion((value) => value + 1);
    } catch (error) {
      if (error instanceof AuthorizationError) setMessage(error.message);
      else { console.error("Digital DEUR submission failed.", error); setMessage("DEUR submission could not be completed. Try again or contact support."); }
    } finally { setPendingAction(undefined); }
  }
  ...
  return <main className="mx-auto max-w-xl space-y-4 p-4 pb-28 sm:p-6">
    ...
    {active && <section className="app-card space-y-4 p-4"><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><div><p className="text-xs text-slate-500">Standby</p><strong>{projection?.valid ? (projection.value.projectedMealBreakMinutes / 60).toFixed(2) : "0.00"} h</strong></div><div><p className="text-xs text-slate-500">Breakdown</p><strong>{projection?.valid ? (projection.value.projectedBreakdownMinutes / 60).toFixed(2) : "0.00"} h</strong></div></div>{access.allowedActions.includes("START_IDLE")&&<div className="space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-3"><label className="block text-sm font-medium">Idle Reason *<select className="app-control mt-1 min-h-12 w-full" value={idleReasonId} onChange={(event)=>setIdleReasonId(event.target.value)}><option value="">Select an active reason</option>{activeIdleReasons.map((reason)=><option key={reason.id} value={reason.id}>{reason.name}</option>)}</select></label>{selectedIdleReason?.requiresRemarks&&<label className="block text-sm font-medium">Remarks *<textarea className="app-control mt-1 w-full" value={idleReasonRemarks} onChange={(event)=>setIdleReasonRemarks(event.target.value)}/></label>}<button type="button" className="min-h-12 w-full rounded-lg bg-blue-700 px-4 py-3 text-base font-semibold text-white disabled:bg-slate-400" disabled={Boolean(pendingAction)||!selectedIdleReason||(Boolean(selectedIdleReason.requiresRemarks)&&!idleReasonRemarks.trim())} onClick={()=>applyAction("START_IDLE")}>Start Idle</button></div>}<div className="grid gap-3 sm:grid-cols-2">{access.allowedActions.filter((action)=>action!=="START_IDLE" && action !== "END_SHIFT").map((action) => <button type="button" disabled={Boolean(pendingAction)} key={action} className="min-h-12 rounded-lg bg-blue-700 px-4 py-3 text-base font-semibold text-white disabled:bg-slate-400" onClick={() => applyAction(action)}>{actionLabels[action]}</button>)}</div>{activityHistory.length>0&&<details className="rounded-lg border bg-slate-50 p-3"><summary className="cursor-pointer font-semibold">View Activity History</summary><ol className="mt-3 space-y-2 text-sm">{activityHistory.map((item)=><li className="rounded border bg-white p-3" key={`${item.sequence}-${item.start}`}><strong>{item.activityType}</strong>{item.idleReasonLabel&&<><br/>Reason: {item.idleReasonLabel}</>}<br/>Start: {new Date(item.start).toLocaleString()}<br/>End: {new Date(item.end).toLocaleString()}<br/>Duration: {item.durationMinutes} minutes{item.remarks&&<><br/>Remarks: {item.remarks}</>}</li>)}</ol></details>}</section>}
    {active && <div className="operator-sticky-actions"><div className="mx-auto flex max-w-xl flex-col gap-2">{active.events?.some((event) => event.activityType === "shift" && event.action === "end") ? <button type="button" disabled={Boolean(pendingAction)} className="min-h-12 w-full rounded-lg bg-emerald-700 px-4 py-3 text-base font-semibold text-white disabled:bg-slate-400" onClick={submit}>{pendingAction === "SUBMIT" ? "Submitting DEUR…" : "Submit DEUR"}</button> : access.allowedActions.includes("END_SHIFT") ? <button type="button" disabled={Boolean(pendingAction)} className="min-h-12 w-full rounded-lg border border-red-500 px-4 py-3 text-base font-semibold text-red-700 disabled:bg-slate-100" onClick={() => applyAction("END_SHIFT")}>End Shift</button> : <p role="status" className="rounded-lg bg-amber-50 p-3 text-sm text-amber-900">End Shift before submitting.</p>}</div></div>}
