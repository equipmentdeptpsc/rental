...
export default function OperatorDeurPage() {
  ...
  return <main className="mx-auto max-w-xl space-y-4 p-4 sm:p-6">
    ...
    {visibleOperatorLines.length > 1 && <label className="block rounded-xl border bg-white p-4 text-sm">Equipment Line<select className="mt-2 w-full rounded border p-3" value={visibleOperatorLines.some((line)=>line.id===selectedLineId)?selectedLineId:""} onChange={(event) => {const value=event.target.value;setSelectedLineId(value);setSearchParams(value?{lineId:value}:{},{replace:true});}}><option value="">Select equipment</option>{visibleOperatorLines.map((line) => { const item = equipment.find((machine) => machine.id === line.equipmentId); const index=operatorLines.findIndex((candidate)=>candidate.id===line.id); return <option key={line.id} value={line.id}>{item ? `${item.equipmentName} (${item.assetNo}) / Rental Line ${index+1}` : `Rental Line ${index+1}`}</option>; })}</select></label>}
    ...
    {active && <section className="space-y-4 rounded-xl border bg-white p-4"><div><span className="text-sm text-slate-500">Current Activity</span><p className="text-xl font-semibold">{projection?.valid ? projection.value.activeEventType === "idle" ? `Idle — ${activeIdleEvent?.idleReasonLabelSnapshot ?? "Reason unavailable"}` : projection.value.activeEventType ?? "Ready" : "Unavailable"}</p>{activeIdleEvent?.timestamp&&<p className="text-sm">Started: {new Date(activeIdleEvent.timestamp).toLocaleTimeString()}</p>}{activeIdleEvent?.idleReasonRemarks&&<p className="text-sm">Remarks: {activeIdleEvent.idleReasonRemarks}</p>}</div><div className="grid grid-cols-2 gap-3"><p><span className="block text-sm text-slate-500">Elapsed</span><strong className="text-2xl tabular-nums">{projection?.valid ? formatElapsed(projection.value.activeEventElapsedSeconds) : "00:00:00"}</strong></p><p><span className="block text-sm text-slate-500">Live / Projected Operation</span><strong>{projection?.valid ? (projection.value.projectedOperationMinutes / 60).toFixed(2) : "0.00"} hours</strong></p></div>{access.allowedActions.includes("START_IDLE")&&<div className="space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-3"><label className="block text-sm font-medium">Idle Reason *<select className="mt-1 min-h-12 w-full rounded border bg-white p-3" value={idleReasonId} onChange={(event)=>setIdleReasonId(event.target.value)}><option value="">Select an active reason</option>{activeIdleReasons.map((reason)=><option key={reason.id} value={reason.id}>{reason.name}</option>)}</select></label>{selectedIdleReason?.requiresRemarks&&<label className="block text-sm font-medium">Remarks *<textarea className="mt-1 w-full rounded border bg-white p-3" value={idleReasonRemarks} onChange={(event)=>setIdleReasonRemarks(event.target.value)}/></label>}<button type="button" className="min-h-12 w-full rounded-lg bg-blue-700 px-4 py-3 font-semibold text-white disabled:bg-slate-400" disabled={!selectedIdleReason||(Boolean(selectedIdleReason.requiresRemarks)&&!idleReasonRemarks.trim())} onClick={()=>applyAction("START_IDLE")}>Start Idle</button></div>}<div className="grid gap-3 sm:grid-cols-2">{access.allowedActions.filter((action)=>action!=="START_IDLE").map((action) => <button key={action} className={`min-h-12 rounded-lg px-4 py-3 font-semibold ${action === "END_SHIFT" ? "border border-red-500 text-red-700" : "bg-blue-700 text-white"}`} onClick={() => applyAction(action)}>{actionLabels[action]}</button>)}</div>{activityHistory.length>0&&<div><h3 className="font-semibold">Activity History</h3><ol className="mt-2 space-y-2 text-sm">{activityHistory.map((item)=><li className="rounded border p-3" key={`${item.sequence}-${item.start}`}><strong>{item.activityType}</strong>{item.idleReasonLabel&&<><br/>Reason: {item.idleReasonLabel}</>}<br/>Start: {new Date(item.start).toLocaleString()}<br/>End: {new Date(item.end).toLocaleString()}<br/>Duration: {item.durationMinutes} minutes{item.remarks&&<><br/>Remarks: {item.remarks}</>}</li>)}</ol></div>}{active.events?.some((event) => event.activityType === "shift" && event.action === "end") && <button className="min-h-12 w-full rounded-lg bg-emerald-700 px-4 py-3 font-semibold text-white" onClick={submit}>Submit DEUR</button>}</section>}
