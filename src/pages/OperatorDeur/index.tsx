...
export default function OperatorDeurPage() {
  const { rentalId = "" } = useParams(); const { user } = useAuth(); const { rentals, rentalEquipmentLines } = useRental(); const { assignments } = useAssignment(); const { operators } = useOperator(); const { equipment } = useEquipment(); const { projects } = useProject(); const { records: workDescriptions } = useWorkDescriptions();
  const rental = rentals.find((item) => item.id === rentalId); const operatorLines = rentalEquipmentLines.filter((line) => line.rentalId === rentalId && ["Released", "Active"].includes(line.status) && (user?.id === line.operatorId || operators.find((item) => item.id === line.operatorId)?.name.toLocaleLowerCase() === user?.name.toLocaleLowerCase()));
  const [selectedLineId, setSelectedLineId] = useState(() => operatorLines.length === 1 ? operatorLines[0].id : ""); const selectedLine = operatorLines.find((line) => line.id === selectedLineId); const assignment = assignments.find((item) => item.id === selectedLine?.assignmentId), operator = operators.find((item) => item.id === selectedLine?.operatorId), machine = equipment.find((item) => item.id === selectedLine?.equipmentId), project = projects.find((item) => item.id === rental?.projectId);
