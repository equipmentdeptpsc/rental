...
export default function OperatorDeurPage() {
  ...
  const [, setVersion] = useState(0), [versions,setVersions]=useState<Record<string,number>>({}), [clock, setClock] = useState(() => new Date().toISOString()), [message, setMessage] = useState(""), [remarks, setRemarks] = useState(""), [pendingAction,setPendingAction]=useState<DeurOperatorAction>();
  ...
  return <main className="mx-auto max-w-xl space-y-4 p-4 pb-28 sm:p-6">
    ...
    {active && <div className="operator-sticky-actions"><div className="mx-auto flex max-w-xl flex-col gap-2">{active.events?.some((event) => event.activityType === "shift" && event.action === "end") ? <button type="button" className="min-h-12 w-full rounded-lg bg-emerald-700 px-4 py-3 text-base font-semibold text-white" onClick={submit}>Submit DEUR</button> : access.allowedActions.includes("END_SHIFT") ? <button type="button" disabled={Boolean(pendingAction)} className="min-h-12 w-full rounded-lg border border-red-500 px-4 py-3 text-base font-semibold text-red-700 disabled:bg-slate-100" onClick={() => applyAction("END_SHIFT")}>End Shift</button> : null}</div></div>}
