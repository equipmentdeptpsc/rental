...
export function createApplicationDependencies(configuration:ApplicationRuntimeConfiguration=readApplicationRuntimeConfiguration(),overrides:ApplicationDependencyOverrides={}):ApplicationDependencies{
  ...
  return{...dependencies,readRepositories:{...readRepositories,equipmentSubcategories,equipmentCategories:new SupabaseEquipmentCategoryReadRepository(client)},commandRepositories:{deurCommands:new SupabaseDeurCommandRepository(client,readRepositories.deurs),...operationalCommands,certificationTypes:new SupabaseCertificationCommandRepository(client),equipmentSubcategories,operatorCertifications:readRepositories.operatorCertifications,canonicalRental:new SupabaseCanonicalRentalRepository(client),...(configuration.remoteOperationalWritesEnabled?{canonicalAssignment:new SupabaseAssignmentCommandRepository(client),canonicalProject:new SupabaseProjectCommandRepository(client),canonicalOperator:new SupabaseOperatorCommandRepository(client),canonicalEquipment:new SupabaseEquipmentCommandRepository(client),canonicalCustomer:new SupabaseCustomerCommandRepository(client)}:{})},changeNotifications:{subscribeDeur:()=>()=>{}},synchronization,authentication:{...dependencies.authentication,remoteAuthenticationProvider,remoteUserAdministration:new SupabaseRemoteUserAdministration(client)},configuration:{equipmentStatusSource:"supabase",persistenceMode:PersistenceMode.Remote,remoteOperationalWritesEnabled:configuration.remoteOperationalWritesEnabled===true}};
}
