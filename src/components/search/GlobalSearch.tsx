...
export default function GlobalSearch() {
  ...
  return <div className="relative"><Search aria-hidden="true" className="pointer-events-none absolute left-3 top-2.5 text-slate-400" size={16} /><input aria-label="Global keyword search" className="w-full rounded-md border border-slate-300 bg-white py-2 pl-9 pr-3 text-xs outline-none placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-900" placeholder="Search rentals, equipment, customers..." value={query} onChange={e => setQuery(e.target.value)} />{query && <div className="absolute right-0 z-50 mt-1 max-h-96 w-full min-w-[340px] overflow-auto rounded border bg-white p-3 shadow-xl dark:border-slate-700 dark:bg-slate-900">{results.length === 0 ? <p className="text-sm text-slate-500">No results found.</p> : groups.map(([type, items]) => <section key={type} className="mb-3"><h3 className="text-xs font-semibold uppercase text-slate-500">{type}</h3>{items.map(item => <Link key={`${item.type}:${item.id}`} className="block rounded p-2 hover:bg-slate-100 dark:hover:bg-slate-800" to={item.href} onClick={() => setQuery("")}><strong className="text-sm">{item.title}</strong><p className="text-xs text-slate-500">{item.subtitle}</p></Link>)}</section>)}</div>}</div>;
}
