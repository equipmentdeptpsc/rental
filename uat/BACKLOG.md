...
| Canonical Return expectation gate | RED | Remote UI exposes Return and the RPC checks open DEUR statuses but not missing immutable expectations. Do not Return until 2026-08-27 receives an authorized actual-DEUR or waiver disposition; then add a forward-only server gate and fail-closed UI readiness projection. |
