...
| Canonical remote Billing creation | REMEDIATION READY | Remote wizard now revalidates canonical server evidence before calling idempotent statement/consumption commands. Local focused 33/33 and full suite 2027 passed, with all TypeScript/build/diff gates green; deployment and exactly-one UAT execution remain. |
| UAT DEFECT INVESTIGATION — Rental Equipment Line / DEUR Relationship | P1 READ-FIRST | Trace schema, commands, revisions, expectations, billing provenance, remote projections, multi-line/workday cardinality, and database enforcement. Do not close based only on the current single-line fixture. |
| Canonical Per Hour precision | REMEDIATION READY | Exact-create guard prevented all writes when server evidence produced PHP 3,883.30 from prematurely rounded hours. Forward migration `20260827000200` prices exact billable hours; clean reset and full 2030-test suite are green. Apply only this migration and recertify PHP 3,883.33. |
