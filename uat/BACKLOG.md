...
| Immutable Billing lineage | REMEDIATION READY | Forward-only `20260828000100` freezes canonical Billing provenance, enforces tenant/line/DEUR/snapshot consistency, and passes clean replay plus rollback-only multi-equipment negative certification. Apply only after scoped commit/push, then certify the existing statement read-only. |
