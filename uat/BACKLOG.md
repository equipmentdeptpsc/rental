...
| Missing 2026-08-25 DEUR expectation | REMEDIATION READY | Initial UAT command failed without mutation because its DEUR guard referenced nonexistent `deleted_at`. Forward-only `20260827000100` uses the canonical current-revision predicate and passes clean replay plus ACCEPTED/REPLAYED runtime cardinality; isolated-UAT apply and one exact approved retry remain. |
