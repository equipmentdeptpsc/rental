...
describe("Rental multi-line billing", () => {
  ...
  it("passes current authorization to persistence and returns diagnosable zero-write failures", () => {
    const source = [deur("1", "line-1", "equipment-1")];
    const statementRecords: any[] = [];
    const deurRecords = new Map(source.map((item) => [item.id, structuredClone(item)]));
    let deurWrites = 0;
    const failed = createRentalLineAwareBillingStatement({
      aggregate: aggregate(source),
      from: "2026-07-01",
      to: "2026-07-31",
      identity: { id: "statement-failed", statementNo: "BS-FAILED" },
      authenticatedUser: administrator,
    }, {
      statements: {
        getByRentalId: () => statementRecords,
        create: () => { throw new Error("simulated persistence rejection"); },
        delete: () => undefined,
      },
      deurs: {
        getById: (id) => structuredClone(deurRecords.get(id)),
        update: (record) => { deurWrites += 1; deurRecords.set(record.id, structuredClone(record)); return record; },
      },
    });
    ...
    expect(failed).toMatchObject({
      ...
      diagnostic: {
        stage: "statement-persistence",
        cause: "simulated persistence rejection",
        statementId: "statement-failed",
        rentalId: "rental-1",
        deurIds: ["1"],
      },
    });
    expect(statementRecords).toEqual([]);
    expect(deurWrites).toBe(0);
    expect(deurRecords.get("1")).not.toMatchObject({ billingLocked: true });
  });

  it("retries successfully once, persists the calculated subtotal, and rejects a duplicate click", () => {
    const source = [deur("1", "line-1", "equipment-1", snapshot(800))];
    source[0].events = [
      { id: "s", activityType: "shift", action: "start", timestamp: "2026-07-02T00:00:00.000Z", sequence: 1, source: "user" },
      { id: "o1", activityType: "operation", action: "start", timestamp: "2026-07-02T01:00:00.000Z", sequence: 2, source: "user" },
      { id: "o2", activityType: "operation", action: "end", timestamp: "2026-07-02T01:10:00.000Z", sequence: 3, source: "user" },
      { id: "e", activityType: "shift", action: "end", timestamp: "2026-07-02T01:10:00.000Z", sequence: 4, source: "user" },
    ];
    source[0].totalOperatingMinutes = 10;
    const statementRecords: any[] = [];
    const deurRecords = new Map(source.map((item) => [item.id, structuredClone(item)]));
    const statements = {
      getByRentalId: () => structuredClone(statementRecords),
      create: (record: any) => { statementRecords.push(structuredClone(record)); },
      delete: (id: string) => {
        const index = statementRecords.findIndex((item) => item.id === id);
        return index >= 0 ? statementRecords.splice(index, 1)[0] : undefined;
      },
    };
    const deurs = {
      getById: (id: string) => structuredClone(deurRecords.get(id)),
      update: (record: DeurRecord) => { deurRecords.set(record.id, structuredClone(record)); return structuredClone(record); },
    };
    const command = {
      aggregate: aggregate(source),
      from: "2026-07-01",
      to: "2026-07-31",
      identity: { id: "statement-retry", statementNo: "BS-RETRY" },
      authenticatedUser: administrator,
    };

    const first = createRentalLineAwareBillingStatement(command, { statements, deurs });
    expect(first).toMatchObject({ success: true, statement: { subtotal: 133.33333333333331 } });
    expect(statementRecords).toHaveLength(1);
    expect(statementRecords[0].subtotal).toBeCloseTo(133.33, 2);
    expect(deurRecords.get("1")).toMatchObject({ billingLocked: true, billingStatementId: "statement-retry" });

    const duplicate = createRentalLineAwareBillingStatement({
      ...command,
      aggregate: { ...command.aggregate, deurs: [...deurRecords.values()] },
    }, { statements, deurs });
    expect(duplicate.success).toBe(false);
    expect(statementRecords).toHaveLength(1);
  });

  it("denies a Management user before statement or DEUR persistence", () => {
    const source = [deur("1", "line-1", "equipment-1")];
    const statementCreate = vi.fn();
    const deurUpdate = vi.fn();
    const result = createRentalLineAwareBillingStatement({
      aggregate: aggregate(source),
      from: "2026-07-01",
      to: "2026-07-31",
      authenticatedUser: { ...administrator, id: "management-1", username: "management", systemRoles: ["management"] },
    }, {
      statements: { getByRentalId: () => [], create: statementCreate, delete: () => undefined },
      deurs: { getById: () => source[0], update: deurUpdate },
    });

    expect(result).toMatchObject({
      success: false,
      code: "AUTHORIZATION_FAILED",
      diagnostic: { stage: "authorization", rentalId: "rental-1", deurIds: ["1"] },
    });
    expect(statementCreate).not.toHaveBeenCalled();
    expect(deurUpdate).not.toHaveBeenCalled();
  });
  ...
  it("bills only eligible lines while reporting incomplete and consumed exclusions", () => {
    storage.clear();
    const eligible = deur("eligible", "line-1", "equipment-1", snapshot(100));
    const incomplete = deur("incomplete", "line-2", "equipment-2", snapshot(250));
    incomplete.status = "In Progress";
    const consumed = deur("consumed", "line-2", "equipment-2", snapshot(250));
    consumed.billingLocked = true;
    consumed.billingStatementId = "older-statement";
    const source = [eligible, incomplete, consumed];
    const input = aggregate(source);
    const preview = buildRentalLineAwareBillingPreview({ aggregate: input, from: "2026-07-01", to: "2026-07-31" });
    expect(preview.lines.map((item) => item.deurId)).toEqual(["eligible"]);
    expect(preview.issues).toContainEqual(expect.objectContaining({ deurId: "incomplete", rentalEquipmentLineId: "line-2" }));
    expect(preview.notices).toHaveLength(1);

    const statements: any[] = [];
    const records = new Map(source.map((item) => [item.id, structuredClone(item)]));
    const result = createRentalLineAwareBillingStatement({
      aggregate: input, from: "2026-07-01", to: "2026-07-31",
      identity: { id: "mixed-statement", statementNo: "BS-MIXED" },
      authenticatedUser: administrator,
    }, {
      statements: {
        getByRentalId: () => statements,
        create: (item) => { statements.push(structuredClone(item)); },
        delete: (id) => {
          const index = statements.findIndex((item) => item.id === id);
          return index >= 0 ? statements.splice(index, 1)[0] : undefined;
        },
      },
      deurs: {
        getById: (id) => structuredClone(records.get(id)),
        update: (item) => { records.set(item.id, structuredClone(item)); return structuredClone(item); },
      },
    });
    expect(result).toMatchObject({ success: true, statement: { id: "mixed-statement", lines: [{ deurId: "eligible", rentalEquipmentLineId: "line-1" }] } });
    expect(records.get("eligible")).toMatchObject({ billingLocked: true, billingStatementId: "mixed-statement" });
    expect(records.get("incomplete")).toMatchObject({ status: "In Progress" });
    expect(records.get("incomplete")?.billingLocked).not.toBe(true);
    expect(records.get("consumed")).toMatchObject({ billingStatementId: "older-statement" });
  });
});
