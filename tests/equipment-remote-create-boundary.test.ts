...
function remoteDependencies(input: { enabled?: boolean; repository?: boolean } = {}) {
  ...
  const equipmentCategories = { list: vi.fn(async () => ({ success: true as const, value: { items: [{ id: "category-1", name: "Earthmoving", active: true, deletedAt: null, sortOrder: 1 }] } })) };
  const equipmentSubcategories = { listAssignable: vi.fn(async (categoryId: string) => ({ success: true as const, value: categoryId === "category-1" ? [{ id: "subcategory-1", categoryId, name: "Excavators", active: true, deletedAt: null, sortOrder: 1 }] : [] })) };
