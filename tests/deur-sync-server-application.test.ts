...
describe("DEUR sync server application service", () => {
  ...
  it("accepts, revisions, orders, and replays valid operations deterministically", async () => {
    const change = conformanceChange("operation-1", "deur-1");
    const first = await service.push({ clientId: "client-a", changes: [change] });
    const replay = await service.push({ clientId: "client-a", changes: [structuredClone(change)] });
  ...
  it("returns stale conflicts with both envelopes and rejects future revisions", async () => {
    await service.push({ clientId: "a", changes: [conformanceChange("create", "deur-1")] });
    const current = { ...conformanceChange("current", "deur-1"), operation: "update" as const, baseRemoteRevision: 1 };
    await service.push({ clientId: "a", changes: [current] });
    const stale = { ...conformanceChange("stale", "deur-1"), operation: "update" as const, baseRemoteRevision: 1 };
    const conflict = await service.push({ clientId: "b", changes: [stale] });
    const future = await service.push({ clientId: "b", changes: [{ ...stale, operationId: "future", idempotencyKey: "future", baseRemoteRevision: 8 }] });
  ...
  it("pulls deterministic ordered pages after numeric cursors", async () => {
    await service.push({ clientId: "a", changes: [conformanceChange("one"), conformanceChange("two"), conformanceChange("three")] });
    const first = await service.pull({ clientId: "b", cursor: "0", limit: 2 });
    const second = await service.pull({ clientId: "b", cursor: first.cursor, limit: 2 });
...
describe("reference controller delegation", () => {
  it("delegates normalized push and pull requests to the application service", async () => {
    const push = vi.fn(async () => ({ accepted: [], rejected: [], conflicts: [], cursor: "0", serverTimestamp: "2026-07-19T10:00:00.000Z" }));
    const pull = vi.fn(async () => ({ changes: [], cursor: "0", hasMore: false, serverTimestamp: "2026-07-19T10:00:00.000Z" }));
