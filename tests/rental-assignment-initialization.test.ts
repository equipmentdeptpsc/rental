...
const project = (id: string, status: ProjectRecord["status"]): ProjectRecord => ({ id, projectCode: `PRJ-${id}`, projectName: id, customerId: "customer-1", location: "", projectManager: "", status });
...
describe("rental assignment initialization", () => {
  ...
  it("keeps active project options selectable and reports invalid inherited projects", () => {
    expect(getRentalProjectOptions([project("project-1", "Active"), project("project-2", "Active"), project("old", "Planning")], "customer-1").map((item) => item.value)).toEqual(["project-1", "project-2"]);
