...
describe("multi-equipment invoice document", () => {
  it("projects every persisted statement value and identity without recalculation", () => {
    ...
    expect(document.serviceLines).toEqual(expect.arrayContaining([expect.objectContaining({ deurReference: "DEUR reference unavailable", service: "Mobilization", amount: 50 })]));
  ...
  it("renders every equipment row, mixed methods, non-zero charges, and print control", () => {
    ...
    expect(html).toContain("Equipment record unavailable"); expect(html).not.toContain("equipment-1"); expect(html).not.toContain("equipment-2"); expect(html).toContain("Rental 1"); expect(html).toContain("Rental 2"); expect(html).toContain("Fuel Charge"); expect(html).not.toContain("Mobilization"); expect(html).toContain("Print");
  });
