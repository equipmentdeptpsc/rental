...
import { projectCanonicalDeurCommercialSnapshots } from "@/features/rental/workspace/projectCanonicalRentalWorkspace";
...
describe("remote Operator DEUR work-date and event projection", () => {
  it("hydrates the exact immutable commercial snapshot and omits database null optionals", () => {
    const result=mapDeur({id:"deur-1",rental_id:"rental-1",commercial_snapshot_id:"snapshot-1",commercial_snapshot_required:true,rental_equipment_line_id:"line-1",equipment_id:"eq",operator_id:"op",work_date:"2026-08-26",status:"Acknowledged",created_at:"2026-08-26T01:00:00Z",updated_at:"2026-08-26T01:00:00Z",deur_events:[]});
    if(!result.success)throw new Error("DEUR mapping failed");
    const [projected]=projectCanonicalDeurCommercialSnapshots([result.value],[{id:"snapshot-1",rentalId:"rental-1",rentalEquipmentLineId:"line-1",sourceContractId:"contract-1",billingMethod:"Per Hour",currency:"PHP",unitRate:1000,minimumBillableHours:null as never,overtimeRate:null as never,standbyRate:null as never,mobilizationFee:null as never,demobilizationFee:null as never,fuelCharge:null as never,operatorIncluded:true,operatorRate:null as never,taxRate:null as never,withholdingTax:null as never,contractAmount:null as never,capturedAt:"2026-08-25T01:00:00Z"}]);
    expect(projected).toMatchObject({commercialSnapshot:{billingMethod:"Per Hour",currency:"PHP",unitRate:1000,operatorIncluded:true,capturedAt:"2026-08-25T01:00:00.000Z"}});
    expect(projected.commercialSnapshot).not.toHaveProperty("minimumBillableHours");
  });
  it("calculates the current UAT fixture read-only from operation intervals and the immutable rate", () => {
    ...
    const mapped=mapDeur({id:"4223f012-9473-4fd5-9575-96cd984ec1a2",deur_number:"DEUR-2026-000001",rental_id:"0ac5c327-2d47-46e9-b94f-2b77deb27427",rental_equipment_line_id:"52ea3624-e8f1-44aa-a89d-02caadf2fe51",commercial_snapshot_id:"snapshot",equipment_id:"equipment",operator_id:"operator",work_date:"2026-08-26",status:"Acknowledged",legacy:false,commercial_snapshot_required:true,total_operating_minutes:0,total_idle_minutes:0,total_maintenance_minutes:0,total_meal_break_minutes:0,total_mobilization_minutes:0,total_demobilization_minutes:0,created_at:"2026-08-26T01:17:39Z",updated_at:"2026-08-26T12:07:39Z",deur_events:events});
    if(!mapped.success)throw new Error("UAT fixture mapping failed");
    const [projected]=projectCanonicalDeurCommercialSnapshots([mapped.value],[{id:"snapshot",rentalId:"0ac5c327-2d47-46e9-b94f-2b77deb27427",rentalEquipmentLineId:"52ea3624-e8f1-44aa-a89d-02caadf2fe51",sourceContractId:"contract",billingMethod:"Per Hour",currency:"PHP",unitRate:1000,operatorIncluded:true,capturedAt:"2026-08-25T01:00:00Z"}]);
    const preview=createDeurBillingPreview({deur:projected,terms:{billingMethod:"Per Hour",unitRate:9999,operatorIncluded:false},evaluatedAt:"2026-08-27T00:00:00Z"});
