...
describe("Operator DEUR route line resolution",()=>{
  ...
  it("returns an actionable missing-operator error and rejects relationship or snapshot mismatches",()=>{
    expect(resolveOperatorDeurRouteLine({...input,lineId:"line-a",operators:[operators[1]]})).toMatchObject({status:"OPERATOR_NOT_FOUND",message:expect.stringContaining("Review the Assignment")});
