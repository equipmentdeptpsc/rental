...
export async function dispatchExistingUatNotification(request:Request,environment:GroupedReviewWorkerEnvironment):Promise<SafeResult>{
 ...
 const administrator=await service.schema("erp").from("user_roles").select("role_id,app_roles!inner(code,active,deprecated_at)").eq("user_id",identity.data.user.id).eq("app_roles.code","system-administrator").eq("app_roles.active",true).is("app_roles.deprecated_at",null).maybeSingle();if(administrator.error||!administrator.data)return safe(403,{success:false,code:"FORBIDDEN"});
