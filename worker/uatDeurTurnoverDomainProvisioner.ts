...
export async function provisionUatDeurTurnoverDomain(request: Request, env: GroupedReviewWorkerEnvironment): Promise<SafeResult> {
  ...
  const claimed = await service.schema("erp").rpc("claim_uat_deur_turnover_domain_scenario", { command: { companyId: tenant, scenarioKey: KEY, profileVersion: PROFILE } });
  ...
  await canonical(client, "command_create_customer", { ...cmd(ids.customerId, "CUSTOMER"), customerId: ids.customerId, customerCode: "UAT-TURNOVER-CUSTOMER-001", name: "Synthetic UAT Turnover Customer 001" });
  ...
  const references = await service.schema("erp").from("cost_codes").select("id").eq("active", true).is("deleted_at", null).order("sort_order", { ascending: true }).limit(1).maybeSingle();
  if (references.error || !references.data) return out(409, { success: false, code: "UAT_REFERENCE_UNAVAILABLE" });
  await canonical(client, "command_create_equipment", { ...cmd(ids.equipmentId, "EQUIPMENT"), equipmentId: ids.equipmentId, assetNo: "UAT-TURNOVER-EQ-001", equipmentName: "Synthetic UAT Turnover Equipment 001", maintenanceType: "Engine Hours", costCodeId: references.data.id, currentReading: 0, remarks: "Synthetic isolated-UAT turnover certification equipment." });
  ...
  await canonical(client, "command_prepare_reserved_rental_aggregate", { ...cmd(ids.rentalId, "PREPARE"), expectedRentalVersion: 1, rentalId: ids.rentalId, lines: [{ lineId: ids.lineId, commercialTerms: { billingMethod: "Per Hour", unitRate: 1000, minimumBillableHours: 0, overtimeRate: 0, standbyRate: 0, mobilizationFee: 0, demobilizationFee: 0, fuelCharge: 0, operatorIncluded: true, operatorRate: 0, taxRate: 0, withholdingTax: 0, contractAmount: 0, currency: "PHP" }, costCodeId: references.data.id, activityCodeId: references.data.id, workDescriptionId: ids.projectId, operationalRemarks: "Synthetic isolated-UAT DEUR turnover runtime certification.", deurPolicy: { frequency: "PER_WORKDAY", effectiveFrom: WORK_DATE }, shiftWindows: [], workDate: WORK_DATE, meterRequirement: "hourMeter" }] });
  await canonical(client, "command_release_rental", { ...cmd(ids.rentalId, "RELEASE"), rentalId: ids.rentalId, expectedVersion: 2 });
  await canonical(client, "command_activate_rental", { ...cmd(ids.rentalId, "ACTIVATE"), rentalId: ids.rentalId, expectedVersion: 3 });
