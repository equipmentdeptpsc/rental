...
export async function provisionUatDeurTurnoverDomain(request: Request, env: GroupedReviewWorkerEnvironment): Promise<SafeResult> {
  ...
  await stage("ASSIGNMENT", () => canonical(client, "command_create_assignment", { ...cmd(ids.assignmentId, "ASSIGNMENT"), assignmentId: ids.assignmentId, equipmentId: ids.equipmentId, operatorId: ids.primaryOperatorId, projectId: ids.projectId, activityCodeId: activityReference.data.id, assignedDate: WORK_DATE, expectedReturn: WORK_DATE, remarks: "Synthetic isolated-UAT turnover certification assignment." }));
