...
export async function provisionUatDeurTurnoverDomain(request: Request, env: GroupedReviewWorkerEnvironment): Promise<SafeResult> {
  ...
  const assignmentPayload = { ...cmd(ids.assignmentId, "ASSIGNMENT"), assignmentId: ids.assignmentId, equipmentId: ids.equipmentId, operatorId: ids.primaryOperatorId, projectId: ids.projectId, activityCodeId: activityReference.data.id, assignedDate: WORK_DATE, expectedReturn: WORK_DATE, remarks: "Synthetic isolated-UAT turnover certification assignment." };
  const assignmentResult = await client.schema("erp").rpc("command_create_assignment", { command: assignmentPayload });
  if (assignmentResult.error || !(assignmentResult.data as Json)?.success) {
    const code = String((assignmentResult.data as Json)?.code ?? "CANONICAL_COMMAND_FAILED");
    if (code !== "IDEMPOTENCY_MISMATCH") throw new Error(`UAT_TURNOVER_SCENARIO_FAILED:ASSIGNMENT:${code}`);
    await stage("ASSIGNMENT_REPAIR", async () => { const repaired = await service.schema("erp").rpc("repair_uat_deur_turnover_assignment_activity", { command: { companyId: tenant, scenarioKey: KEY, profileVersion: PROFILE, assignmentId: ids.assignmentId, projectId: ids.projectId, operatorId: ids.primaryOperatorId, equipmentId: ids.equipmentId, activityCodeId: activityReference.data.id } }); if (repaired.error || !(repaired.data as Json)?.success) throw new Error(String((repaired.data as Json)?.code ?? "REPAIR_FAILED")); });
  }
