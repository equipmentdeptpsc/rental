...
export default{
 async fetch(request:Request,environment:GroupedReviewWorkerEnvironment):Promise<Response>{
  ...
  if(path==="/api/admin/uat/preflight-grouped-review-certification"){
   ...
   if(path==="/api/admin/uat/provision-limited-operational-pilot"||path==="/api/admin/uat/inspect-limited-operational-pilot"){
     const cors=uatAdminCorsHeaders(request,environment);
     if(request.method==="OPTIONS")return new Response(null,{status:204,headers:{...cors,"cache-control":"no-store"}});
     if(request.method!=="POST")return Response.json({success:false,code:"METHOD_NOT_ALLOWED"},{status:405,headers:{...cors,allow:"POST","cache-control":"no-store"}});
   ...
   if(path==="/api/admin/uat/inspect-limited-pilot-scenario1"){
     const cors=uatAdminCorsHeaders(request,environment); if(request.method==="OPTIONS")return new Response(null,{status:204,headers:{...cors,"cache-control":"no-store"}}); if(request.method!=="POST")return Response.json({success:false,code:"METHOD_NOT_ALLOWED"},{status:405,headers:{...cors,allow:"POST","cache-control":"no-store"}}); try{const result=await inspectUatLimitedPilotScenario1(request,environment);return Response.json(result.body,{status:result.status,headers:{...cors,"cache-control":"no-store"}});}catch{return Response.json({success:false,code:"UAT_LIMITED_PILOT_SCENARIO1_READ_FAILED"},{status:503,headers:{...cors,"cache-control":"no-store"}});}
   }
  if(path==="/api/admin/uat/inspect-limited-pilot-scenarios"){
     const cors=uatAdminCorsHeaders(request,environment); if(request.method==="OPTIONS")return new Response(null,{status:204,headers:{...cors,"cache-control":"no-store"}}); if(request.method!=="POST")return Response.json({success:false,code:"METHOD_NOT_ALLOWED"},{status:405,headers:{...cors,allow:"POST","cache-control":"no-store"}}); try{const result=await inspectUatLimitedPilotScenarios(request,environment);return Response.json(result.body,{status:result.status,headers:{...cors,"cache-control":"no-store"}});}catch{return Response.json({success:false,code:"UAT_LIMITED_PILOT_MULTI_READ_FAILED"},{status:503,headers:{...cors,"cache-control":"no-store"}});}
  }
  ...
  if(path==="/api/admin/uat/inspect-limited-operational-pilot-deurs"||path==="/api/admin/uat/inspect-limited-pilot-deurs"){
  ...
  if(path==="/api/admin/uat/inspect-limited-pilot-daily-eligibility"){
    const cors=uatAdminCorsHeaders(request,environment); if(request.method==="OPTIONS")return new Response(null,{status:204,headers:{...cors,"cache-control":"no-store"}}); if(request.method!=="POST")return Response.json({success:false,code:"METHOD_NOT_ALLOWED"},{status:405,headers:{...cors,allow:"POST","cache-control":"no-store"}}); try{const result=await inspectUatLimitedPilotDailyEligibility(request,environment);return Response.json(result.body,{status:result.status,headers:{...cors,"cache-control":"no-store"}});}catch{return Response.json({success:false,code:"UAT_LIMITED_PILOT_DAILY_ELIGIBILITY_READ_FAILED"},{status:503,headers:{...cors,"cache-control":"no-store"}});}
  }
