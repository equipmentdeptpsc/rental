...
export async function runUatGroupedReviewCertification(request:Request,environment:GroupedReviewWorkerEnvironment):Promise<SafeResult>{
 ...
 const residue=await service.schema("erp").from("notification_outbox").select("id,status,provider_message_id,last_failure_category").eq("company_id","TENANT-LOCAL-001").eq("notification_type","CUSTOMER_GROUPED_REVIEW_REQUESTED");
 if(residue.error)return safe(503,{success:false,code:"CANONICAL_EVIDENCE_UNAVAILABLE"});
 let scheduler:unknown={groupsPrepared:0,notificationsPrepared:0};
 const deadletters=(residue.data??[]).filter(item=>item.status==="DeadLetter"&&item.last_failure_category==="AuthenticationFailure"&&!item.provider_message_id);
 if(deadletters.length===1&&(residue.data??[]).every(item=>item.status!=="ProviderAccepted")){
