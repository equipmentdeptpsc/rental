...
export async function runUatGroupedReviewCertification(request:Request,environment:GroupedReviewWorkerEnvironment):Promise<SafeResult>{
 ...
 const body=await request.json().catch(()=>null) as Record<string,unknown>|null;const rentalId=String(body?.rentalId??""),requestedDeurId=String(body?.deurId??""),deurNumber=String(body?.deurNumber??""),workDate=String(body?.workDate??""),timezone=String(body?.timezone??"");
 ...
 if(!deurId){const resolved=await service.schema("erp").from("deurs").select("id").eq("rental_id",rentalId).eq("deur_number",deurNumber).eq("work_date",workDate).is("superseded_by_revision_id",null).limit(2);if(resolved.error||resolved.data?.length!==1)return safe(409,{success:false,code:"TARGET_NOT_ELIGIBLE"});deurId=String((resolved.data[0] as {id?:unknown}).id??"");if(!/^[0-9a-f-]{36}$/i.test(deurId))return safe(409,{success:false,code:"TARGET_NOT_ELIGIBLE"});}
 ...
 const residue=await service.schema("erp").rpc("certify_isolated_uat_grouped_review_residue",{command:{rentalId,deurId}});const residueResult=residue.data as{success?:boolean;value?:{notifications?:Array<{id:string;status:string;providerMessageIdPresent:boolean;failureCategory?:string}>}}|null;
 if(residue.error||!residueResult?.success)return safe(503,{success:false,code:"CANONICAL_EVIDENCE_UNAVAILABLE"});const notifications=residueResult.value?.notifications??[];
