...
export async function runUatGroupedReviewCertification(request:Request,environment:GroupedReviewWorkerEnvironment):Promise<SafeResult>{
 ...
 if(!deurId){const resolved=await service.schema("erp").from("deurs").select("id").eq("rental_id",rentalId).eq("deur_number",deurNumber).eq("work_date",workDate).is("superseded_by_revision_id",null).limit(2);if(resolved.error)return safe(503,{success:false,code:"TARGET_RESOLUTION_UNAVAILABLE"});if(resolved.data?.length!==1)return safe(409,{success:false,code:"TARGET_RESOLUTION_NOT_UNIQUE"});deurId=String((resolved.data[0] as {id?:unknown}).id??"");if(!/^[0-9a-f-]{36}$/i.test(deurId))return safe(503,{success:false,code:"TARGET_RESOLUTION_UNAVAILABLE"});}
 ...
 if(mode==="PREFLIGHT")return safe(200,{success:true,result:"ELIGIBLE"});
