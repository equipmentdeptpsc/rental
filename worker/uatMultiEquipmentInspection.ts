...
export async function inspectUatMultiEquipmentProvisioning(request:Request,environment:GroupedReviewWorkerEnvironment):Promise<SafeResult>{
 ...
 const inspected=await service.schema("erp").rpc("inspect_isolated_uat_multi_equipment_scenario",{target_tenant:companyId,target_scenario:scenarioKey});if(inspected.error)return safe(409,{success:false,code:"INSPECTION_UNAVAILABLE"});return safe(200,inspected.data as Record<string,unknown>);
}
