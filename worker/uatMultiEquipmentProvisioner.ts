...
import {readUatUpstreamReplayLineage} from "./uatUpstreamReplayLineage";
...
export async function provisionUatMultiEquipmentCertification(request:Request,environment:GroupedReviewWorkerEnvironment):Promise<SafeResult>{
 ...
 if(!draft.costCodeId||!draft.activityCodeId){const code=String(refs.code||"UAT_REFERENCE_UNAVAILABLE");await persistDiagnostic(service,companyId,actorId,scenarioKey,"REFERENCE_VALIDATION","resolve_isolated_uat_multi_equipment_references",code);await service.schema("erp").rpc("finish_isolated_uat_provisioning_attempt",{command:{attemptId,actorId,state:"FAILED",safeResultCode:code}});return bad(code);}
 ...
 try{
  const upstream=await readUatUpstreamReplayLineage(service,companyId,scenarioKey); if(upstream.readStatus!=="SUCCESS"||upstream.decision!=="SAFE") throw new Error("upstream_lineage::UAT_UPSTREAM_LINEAGE_READ_FAILED");
  if(upstream.customer?.classification==="ABSENT")await rpc(user,"command_create_customer",{command:{...command(scenario.customerId,"CUSTOMER"),customerId:scenario.customerId,customerCode:"UAT-ME-CERT-20260829",name:"Synthetic UAT Multi-Equipment Customer"}});
  if(upstream.project?.classification==="ABSENT")await rpc(user,"command_create_project",{command:{...command(scenario.projectId,"PROJECT"),projectId:scenario.projectId,projectCode:"UAT-ME-CERT-20260829",name:"Synthetic UAT Multi-Equipment Project",customerId:scenario.customerId,location:"Isolated UAT"}});
  if(upstream.workDescription?.classification==="ABSENT")await rpc(user,"command_create_work_description",{command:{...command(scenario.workDescriptionId,"WORK"),workDescriptionId:scenario.workDescriptionId,code:"UAT-ME-RUNTIME-CERT",name:"Synthetic isolated-UAT multi-equipment runtime certification.",requiresRemarks:false,sortOrder:999}});
  for(let i=0;i<3;i++)if(upstream.operators[i]?.classification==="ABSENT")await rpc(user,"command_create_operator",{command:{...command(scenario.operatorIds[i],`OPERATOR-${i+1}`),operatorId:scenario.operatorIds[i],name:`Synthetic UAT Multi-Equipment Operator ${i+1}`,certificationType:"Heavy Machinery",joinedDate:workDate}});
  for(let i=0;i<3;i++)if(upstream.equipment[i]?.classification==="ABSENT")await rpc(user,"command_create_equipment",{command:{...command(scenario.equipmentIds[i],`EQUIPMENT-${i+1}`),equipmentId:scenario.equipmentIds[i],assetNo:`UAT-ME-20260829-${i+1}`,equipmentName:`Synthetic UAT Multi-Equipment ${i+1}`,maintenanceType:"Engine Hours",costCodeId:scenario.costCodeId,currentReading:0,remarks:"Synthetic isolated-UAT certification equipment."}});
  for(let i=0;i<3;i++)if(upstream.assignments[i]?.classification==="ABSENT")await rpc(user,"command_create_assignment",{command:{...command(scenario.assignmentIds[i],`ASSIGNMENT-${i+1}`),assignmentId:scenario.assignmentIds[i],equipmentId:scenario.equipmentIds[i],operatorId:scenario.operatorIds[i],projectId:scenario.projectId,activityCodeId:scenario.activityCodeId,assignedDate:workDate,expectedReturn:workDate,remarks:"Synthetic isolated-UAT certification assignment."}});
  await reservePrepareReleaseActivate(user,service,companyId,scenario,"A",scenario.rentalAId,scenario.rentalALineIds,[0,1],true);
  await reservePrepareReleaseActivate(user,service,companyId,scenario,"B",scenario.rentalBId,[scenario.rentalBLineId],[2],true);
...
async function reservePrepareReleaseActivate(client:any,readClient:any,companyId:string,scenario:Scenario,label:string,rentalId:string,lineIds:string[],indices:number[],allowReuse=false){
 ...
 let version=0;
 if(allowReuse){ const partial=await readUatPartialRentalLineage(readClient,companyId,scenarioKey,rentalId); if(partial.status!=="SUCCESS"&&partial.status!=="RENTAL_NOT_FOUND") throw new Error("lineage_read::UAT_PARTIAL_RENTAL_LINEAGE_READ_FAILED"); const rental=partial.rental as any; const existingLines=partial.lines as any[]; if(rental){ const expected=new Set(lineIds), observed=existingLines.map((line:any)=>line.id); const exact=String(rental.id)===rentalId&&rental.rental_number===rentalNumber&&rental.status==="Reserved"&&rental.company_id===companyId&&rental.customer_id===scenario.customerId&&rental.project_id===scenario.projectId&&String(rental.date_out).slice(0,10)===workDate&&String(rental.expected_return).slice(0,10)===workDate&&observed.length===lineIds.length&&observed.every((id:string)=>expected.has(id))&&existingLines.every((line:any)=>line.status==="Reserved"&&line.company_id===companyId&&line.equipment_id===scenario.equipmentIds[indices[lineIds.indexOf(line.id)]]&&line.assignment_id===scenario.assignmentIds[indices[lineIds.indexOf(line.id)]]&&line.operator_id===scenario.operatorIds[indices[lineIds.indexOf(line.id)]]); if(!exact)throw new Error("lineage_read::UAT_PARTIAL_RENTAL_LINEAGE_MISMATCH"); version=Number(rental.row_version); }} if(version===0){
