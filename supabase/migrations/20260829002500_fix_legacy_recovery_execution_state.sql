...
BEGIN
  ...
  IF attempts<>0 OR EXISTS(SELECT 1 FROM erp.uat_multi_equipment_execution_attempts a WHERE a.company_id=tenant AND a.scenario_key=scenario_key_value AND a.state='RUNNING' AND a.lease_expires_at>clock_timestamp())
