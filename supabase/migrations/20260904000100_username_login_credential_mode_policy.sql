...
CREATE OR REPLACE FUNCTION erp.resolve_active_application_user_login(identifier text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  match_count integer;
  resolved_email text;
  resolved_credential_mode text;
BEGIN
  IF normalized_identifier = '' OR length(normalized_identifier) > 120 THEN
    RETURN jsonb_build_object('success', false);
  END IF;

  SELECT count(*)::integer, min(lower(auth_user.email)), min(application_user.credential_mode)
  INTO match_count, resolved_email, resolved_credential_mode
  ...
  RETURN jsonb_build_object('success', true, 'email', resolved_email, 'credentialMode', coalesce(resolved_credential_mode, 'PASSWORD'));
END $$;
