...
CREATE OR REPLACE FUNCTION erp.search_booking_rows(
  ...
  p_rental_number_search text DEFAULT NULL,
  p_order_field text DEFAULT 'createdAt',
...
BEGIN
  ...
  WITH rows AS (
    SELECT
      ...
      rental.status::text AS rental_status,
      line.id AS rental_equipment_line_id,
      line.equipment_id,
    ...
    JOIN erp.equipment AS equipment
      ON equipment.id = line.equipment_id
      AND equipment.company_id = line.company_id
      AND equipment.deleted_at IS NULL
    LEFT JOIN erp.customers AS customer
      ON customer.id = rental.customer_id
      AND customer.company_id = rental.company_id
      AND customer.deleted_at IS NULL
    LEFT JOIN erp.projects AS project
      ON project.id = rental.project_id
      AND project.company_id = rental.company_id
  ...
  SELECT * FROM counted
  ORDER BY
    CASE WHEN v_order_field = 'createdAt' AND p_order_ascending THEN created_at END ASC NULLS LAST,
    CASE WHEN v_order_field = 'createdAt' AND NOT p_order_ascending THEN created_at END DESC NULLS LAST,
    CASE WHEN v_order_field = 'dateOut' AND p_order_ascending THEN date_out END ASC NULLS LAST,
    CASE WHEN v_order_field = 'dateOut' AND NOT p_order_ascending THEN date_out END DESC NULLS LAST,
    CASE WHEN v_order_field = 'expectedReturn' AND p_order_ascending THEN expected_return END ASC NULLS LAST,
    CASE WHEN v_order_field = 'expectedReturn' AND NOT p_order_ascending THEN expected_return END DESC NULLS LAST,
    CASE WHEN v_order_field = 'rentalStatus' AND p_order_ascending THEN rental_status END ASC NULLS LAST,
    CASE WHEN v_order_field = 'rentalStatus' AND NOT p_order_ascending THEN rental_status END DESC NULLS LAST,
    rental_equipment_line_id DESC
  OFFSET v_offset
  LIMIT v_limit;
END;
$$;
...
COMMENT ON FUNCTION erp.search_booking_rows(text, text, text, text, text, text, boolean, integer, integer) IS
  'Read-only, tenant-derived Rental Equipment Line booking projection. Requires rental.read, conditionally nulls secondary labels by their read permissions, excludes deleted lines/equipment, clamps paging, and accepts no client company scope.';
