...
-- This is support-state for one deliberately narrow compatibility-UAT scenario.
-- It contains no credentials and is not a general rental provisioning surface.
CREATE TABLE erp.uat_multi_equipment_provisioning_scenarios (
...
ALTER TABLE erp.uat_multi_equipment_provisioning_scenarios ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON erp.uat_multi_equipment_provisioning_scenarios FROM PUBLIC,anon,authenticated;

CREATE FUNCTION erp.claim_isolated_uat_multi_equipment_provisioning(command jsonb) RETURNS jsonb
...
BEGIN
 IF tenant<>'TENANT-LOCAL-001' OR scenario_key_value<>'MULTI-EQUIPMENT-RUNTIME-CERT-2026-08-29' OR profile_value<>'UAT_MULTI_EQUIPMENT_PER_WORKDAY_V1' OR actor IS NULL OR jsonb_typeof(command->'scenario')<>'object'
...
CREATE FUNCTION erp.complete_isolated_uat_multi_equipment_provisioning(command jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=erp,auth,extensions,pg_catalog AS $$
...
ALTER FUNCTION erp.claim_isolated_uat_multi_equipment_provisioning(jsonb) OWNER TO postgres;
ALTER FUNCTION erp.complete_isolated_uat_multi_equipment_provisioning(jsonb) OWNER TO postgres;
REVOKE ALL ON FUNCTION erp.claim_isolated_uat_multi_equipment_provisioning(jsonb),erp.complete_isolated_uat_multi_equipment_provisioning(jsonb) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION erp.claim_isolated_uat_multi_equipment_provisioning(jsonb),erp.complete_isolated_uat_multi_equipment_provisioning(jsonb) TO service_role;
COMMIT;
