...
BEGIN
  IF TG_OP = 'UPDATE' AND OLD.is_open = true AND NEW.is_open = false
     AND (to_jsonb(NEW) - 'is_open') = (to_jsonb(OLD) - 'is_open') THEN RETURN NEW; END IF;
  ...
  IF TG_OP = 'DELETE' AND session_user = database_owner AND current_user = database_owner AND (
    ...
    (current_setting('erp.c12_manager_email_fixture_cleanup', true) = 'TENANT-UAT-C12-MANAGER-EMAIL-001' AND OLD.company_id = 'TENANT-UAT-C12-MANAGER-EMAIL-001')
...
BEGIN
  ...
  IF TG_OP = 'DELETE' AND TG_TABLE_SCHEMA = 'erp'
     ...
     AND session_user = database_owner AND current_user = database_owner AND (
       ...
       (current_setting('erp.c12_manager_email_fixture_cleanup', true) = 'TENANT-UAT-C12-MANAGER-EMAIL-001' AND target_company = 'TENANT-UAT-C12-MANAGER-EMAIL-001')
     ) THEN RETURN OLD; END IF;
  RAISE EXCEPTION 'immutable historical record cannot be changed' USING ERRCODE = '55000';
END $$;
...
CREATE OR REPLACE FUNCTION erp.reject_manager_review_outcome_change()
RETURNS trigger LANGUAGE plpgsql SET search_path = erp, pg_catalog AS $$
DECLARE database_owner name;
BEGIN
  SELECT pg_get_userbyid(datdba) INTO database_owner FROM pg_database WHERE datname = current_database();
...
CREATE FUNCTION erp.cleanup_c12_manager_real_email_fixture(
...
BEGIN
  ...
  IF (SELECT count(*) FROM companies WHERE id = target_tenant_id) > 1
     OR (SELECT count(*) FROM users WHERE company_id = target_tenant_id) > 2
     OR (SELECT count(*) FROM user_roles ur JOIN users u ON u.id = ur.user_id WHERE u.company_id = target_tenant_id) > 2
...
REVOKE ALL ON FUNCTION erp.protect_deur_event_history(), erp.reject_immutable_change(),
  erp.reject_customer_review_evidence_change(), erp.reject_manager_review_outcome_change(),
  erp.reject_terminal_notification_change(), erp.cleanup_c12_manager_real_email_fixture(text,text,text)
  FROM PUBLIC, anon, authenticated, service_role;

COMMENT ON FUNCTION erp.cleanup_c12_manager_real_email_fixture(text,text,text) IS
  'Owner-only exact cleanup for TENANT-UAT-C12-MANAGER-EMAIL-001 Manager real-email certification; aggregate counts only; auth.users excluded.';
