...
-- Calendar-specific, read-only extension of the certified Rental Equipment Line
-- projection. The shared helper keeps tenant, permission, label, filter, sort,
-- and paging behavior aligned with the existing Booking list RPC.
CREATE FUNCTION erp._search_booking_rows_filtered(
...
BEGIN
  ...
  END IF;
  IF (p_window_start IS NULL) <> (p_window_end IS NULL) THEN
    RAISE EXCEPTION 'calendar window start and end are both required' USING ERRCODE = '22023';
  END IF;
  IF p_window_start IS NOT NULL AND p_window_start > p_window_end THEN
    RAISE EXCEPTION 'calendar window start must not follow end' USING ERRCODE = '22023';
  END IF;
  IF p_window_start IS NOT NULL AND p_window_end - p_window_start > 92 THEN
    RAISE EXCEPTION 'calendar window must not exceed 93 days' USING ERRCODE = '22023';
  END IF;
  ...
  WITH rows AS (
    ...
    WHERE line.deleted_at IS NULL AND erp.can_read_company_row(rental.company_id)
      ...
      -- date_out is required canonical business date. A missing expected_return
      -- is a start-date marker only unless an authoritative actual_return exists.
      AND (p_window_start IS NULL OR (rental.date_out <= p_window_end
        AND COALESCE(rental.actual_return, rental.expected_return, rental.date_out) >= p_window_start))
...
-- Preserve the existing private helper signature and therefore the certified
-- public search_booking_rows contract.
...
AS $$
BEGIN
  RETURN QUERY SELECT * FROM erp._search_booking_rows_filtered(
    p_status, p_customer_id, p_project_id, p_equipment_id, p_rental_number_search,
    p_order_field, p_order_ascending, p_offset, p_limit, NULL, NULL
  );
END;
$$;

CREATE FUNCTION erp.search_booking_calendar_rows(
  p_window_start date, p_window_end date,
  p_status text DEFAULT NULL, p_customer_id text DEFAULT NULL, p_project_id text DEFAULT NULL,
  p_equipment_id text DEFAULT NULL, p_rental_number_search text DEFAULT NULL,
  p_order_field text DEFAULT 'dateOut', p_order_ascending boolean DEFAULT true,
  p_offset integer DEFAULT 0, p_limit integer DEFAULT 25
)
RETURNS TABLE (
...
BEGIN
  RETURN QUERY SELECT * FROM erp._search_booking_rows_filtered(
    p_status, p_customer_id, p_project_id, p_equipment_id, p_rental_number_search,
    p_order_field, p_order_ascending, p_offset, p_limit, p_window_start, p_window_end
  );
END;
$$;

ALTER FUNCTION erp._search_booking_rows_filtered(text,text,text,text,text,text,boolean,integer,integer,date,date) OWNER TO postgres;
ALTER FUNCTION erp._search_booking_rows(text,text,text,text,text,text,boolean,integer,integer) OWNER TO postgres;
ALTER FUNCTION erp.search_booking_calendar_rows(date,date,text,text,text,text,text,text,boolean,integer,integer) OWNER TO postgres;
REVOKE ALL ON FUNCTION erp._search_booking_rows_filtered(text,text,text,text,text,text,boolean,integer,integer,date,date) FROM PUBLIC, anon, authenticated;
...
REVOKE ALL ON FUNCTION erp.search_booking_calendar_rows(date,date,text,text,text,text,text,text,boolean,integer,integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION erp.search_booking_calendar_rows(date,date,text,text,text,text,text,text,boolean,integer,integer) TO authenticated;

COMMENT ON FUNCTION erp.search_booking_calendar_rows(date,date,text,text,text,text,text,text,boolean,integer,integer) IS
  'Read-only, tenant-derived Rental Equipment Line calendar projection. Date overlap uses date_out through actual_return, expected_return, or date_out; inclusive window maximum is 93 days.';
