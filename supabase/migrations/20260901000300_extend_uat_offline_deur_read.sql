...
-- Fixed-scenario, read-only projection for the online baseline of the offline
-- runtime certification.  This intentionally does not expose an arbitrary DEUR
-- lookup or grant access to the underlying tables.
...
BEGIN
  ...
  SELECT count(*) INTO scenario_deur_count
   ...
   WHERE d.company_id=tenant
     ...
     AND d.work_date=(v->>'workDate')::date;
  ...
  IF target_deur.id IS NOT NULL THEN
    SELECT count(*) INTO duplicate_daily_deur_count
     ...
     WHERE d.company_id=tenant
       ...
       AND d.work_date=target_deur.work_date
