...
CREATE OR REPLACE FUNCTION erp.command_prepare_reserved_rental_aggregate(command jsonb)
RETURNS jsonb
...
BEGIN
  ...
  IF NOT erp.current_user_has_permission('rental.update') THEN RETURN jsonb_build_object('success',false,'code','FORBIDDEN','message','Rental management permission is required.','retryable',false,'refreshRequired',false); END IF;
