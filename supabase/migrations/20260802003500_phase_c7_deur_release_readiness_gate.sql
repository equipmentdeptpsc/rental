...
CREATE OR REPLACE FUNCTION rental_release_readiness(target_rental_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=erp,auth
AS $$
DECLARE tenant text; target rentals; incomplete jsonb; active_count integer;
BEGIN
  tenant=(SELECT company_id FROM users WHERE id=auth.uid() AND status='active');
  IF tenant IS NULL THEN RETURN jsonb_build_object('eligible',false,'reasonCodes',jsonb_build_array('UNAUTHENTICATED'),'rentalId',target_rental_id,'incompleteEquipmentLines','[]'::jsonb); END IF;
  IF NOT (current_user_has_permission('rental.manage') OR current_user_has_permission('rental.release')) THEN RETURN jsonb_build_object('eligible',false,'reasonCodes',jsonb_build_array('FORBIDDEN'),'rentalId',target_rental_id,'incompleteEquipmentLines','[]'::jsonb); END IF;
  SELECT * INTO target FROM rentals WHERE id=target_rental_id AND company_id=tenant;
  IF target.id IS NULL THEN RETURN jsonb_build_object('eligible',false,'reasonCodes',jsonb_build_array('NOT_FOUND'),'rentalId',target_rental_id,'incompleteEquipmentLines','[]'::jsonb); END IF;
  ...
  SELECT coalesce(jsonb_agg(problem ORDER BY problem->>'rentalEquipmentLineId'),'[]'::jsonb) INTO incomplete
  FROM (
    SELECT jsonb_build_object(
      ...
      'missingFields',to_jsonb(array_remove(ARRAY[
        ...
        CASE WHEN p.id IS NULL OR p.status<>'Active' THEN 'project' END,
...
CREATE OR REPLACE FUNCTION command_release_rental(command jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=erp,auth AS $$
DECLARE tenant text; readiness jsonb; result jsonb; now_at timestamptz=clock_timestamp();
BEGIN
  tenant=(SELECT company_id FROM users WHERE id=auth.uid() AND status='active');
  ...
  IF NOT current_user_has_permission('rental.release') THEN RETURN jsonb_build_object('success',false,'code','FORBIDDEN','message','Rental release permission is required.','retryable',false,'refreshRequired',false); END IF;
  PERFORM id FROM rentals WHERE id=command->>'rentalId' AND company_id=tenant FOR UPDATE;
  PERFORM id FROM rental_equipment_lines WHERE rental_id=command->>'rentalId' AND company_id=tenant AND deleted_at IS NULL ORDER BY id FOR UPDATE;
  readiness=rental_release_readiness(command->>'rentalId');
  IF coalesce((readiness->>'eligible')::boolean,false)=false THEN
    RETURN jsonb_build_object('success',false,'code','RELEASE_NOT_READY','message','Every active equipment line must have a complete, current DEUR release snapshot.','retryable',false,'refreshRequired',false,'readiness',readiness);
  END IF;
  result=execute_rental_lifecycle_transition(command,'RELEASE_RENTAL','Reserved','Released','rental.release');
  IF coalesce((result->>'success')::boolean,false) THEN
    UPDATE rentals SET deur_expectation_frozen_at=coalesce(deur_expectation_frozen_at,now_at) WHERE id=command->>'rentalId' AND company_id=tenant;
    UPDATE rental_equipment_lines SET operational_metadata=jsonb_set(operational_metadata,'{deurExpectationSnapshot,capturedAt}',to_jsonb(coalesce(operational_metadata#>>'{deurExpectationSnapshot,capturedAt}',now_at::text)),true) WHERE rental_id=command->>'rentalId' AND company_id=tenant AND deleted_at IS NULL AND status='Released';
  END IF;
  RETURN result;
END $$;

REVOKE ALL ON FUNCTION rental_release_readiness(text) FROM PUBLIC,anon;
REVOKE ALL ON FUNCTION command_release_rental(jsonb) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION rental_release_readiness(text),command_release_rental(jsonb) TO authenticated;
COMMIT;
