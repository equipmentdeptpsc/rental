...
DECLARE s erp.uat_multi_equipment_provisioning_scenarios; rental_count int; line_count int; deur_count int; billing_count int; execution jsonb;
BEGIN
 ...
 execution:=erp.inspect_isolated_uat_execution(jsonb_build_object('companyId',target_tenant,'scenarioKey',target_scenario));
 IF s.scenario_key IS NULL THEN RETURN jsonb_build_object('success',true,'state','NOT_STARTED','scenarioKey',target_scenario,'profile','UAT_MULTI_EQUIPMENT_PER_WORKDAY_V1','activeExecution',execution->>'activeExecution','executionEvidence',execution->>'executionEvidence','counts',jsonb_build_object('residue',0)); END IF;
