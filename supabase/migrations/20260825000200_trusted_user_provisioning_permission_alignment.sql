...
-- The trusted Worker must be able to verify role assignment eligibility, but
-- these columns remain unavailable to browser roles through this migration.
GRANT SELECT (active, deprecated_at) ON erp.app_roles TO service_role;

CREATE OR REPLACE FUNCTION erp.lookup_application_user_provisioning_command(command jsonb) RETURNS jsonb
...
BEGIN
 IF actor IS NULL OR tenant IS NULL OR idem='' OR NOT EXISTS(SELECT 1 FROM erp.users u WHERE u.id=actor AND u.company_id=tenant AND u.status='active')
    OR NOT EXISTS(SELECT 1 FROM erp.effective_user_permissions p WHERE p.user_id=actor AND p.permission_code='users.create')
    OR NOT EXISTS(SELECT 1 FROM erp.effective_user_permissions p WHERE p.user_id=actor AND p.permission_code='roles.assign')
...
CREATE OR REPLACE FUNCTION erp.command_provision_application_user(command jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=erp,pg_catalog AS $$
DECLARE
...
BEGIN
  ...
  IF cardinality(roles)=0 OR EXISTS(
    SELECT 1 FROM unnest(roles) requested
    WHERE NOT EXISTS(SELECT 1 FROM erp.app_roles r WHERE r.code=requested AND r.active AND r.deprecated_at IS NULL)
  ...
  FOREACH role_code IN ARRAY roles LOOP
    ...
    SELECT target_id,r.id,actor::text FROM erp.app_roles r
    WHERE r.code=role_code AND r.active AND r.deprecated_at IS NULL
...
CREATE OR REPLACE FUNCTION erp.record_application_user_password_reset(command jsonb) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=erp,pg_catalog AS $$
...
BEGIN
 IF actor IS NULL OR target IS NULL OR NOT EXISTS(SELECT 1 FROM erp.users u WHERE u.id=actor AND u.company_id=tenant AND u.status='active') OR NOT EXISTS(SELECT 1 FROM erp.effective_user_permissions p WHERE p.user_id=actor AND p.permission_code='users.password.reset') THEN RETURN jsonb_build_object('success',false,'code','FORBIDDEN');END IF;
