...
BEGIN
 ...
 IF exact_deur IS NOT NULL THEN SELECT count(*) INTO activity_count FROM erp.deur_events ev WHERE ev.deur_id=(exact_deur->>'id') AND ev.activity_type<>'shift'; SELECT count(*) INTO open_activity_count FROM erp.deur_events ev WHERE ev.deur_id=(exact_deur->>'id') AND ev.activity_type<>'shift' AND ev.is_open; END IF;
 ...
 RETURN jsonb_build_object('success',true,'readStatus','SUCCESS','scenarioDeurCount',total_count,'lineCounts',line_rows,'deur',exact_deur,'activityTimelineCount',coalesce(activity_count,0),'activeActivityCount',coalesce(open_activity_count,0),'currentActivity',CASE WHEN coalesce(open_activity_count,0)=0 THEN 'NONE' ELSE 'OPEN' END);
