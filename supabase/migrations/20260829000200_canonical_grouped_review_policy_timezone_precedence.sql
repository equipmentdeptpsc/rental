...
CREATE OR REPLACE FUNCTION erp.certify_isolated_uat_grouped_review_target(command jsonb) RETURNS jsonb
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=erp,auth,pg_catalog AS $$
DECLARE target_rental_id text;target_deur_id text;target_work_date date;
BEGIN
 IF auth.role()<>'service_role' OR jsonb_typeof(command)<>'object'
   ...
   OR coalesce(command->>'workDate','')!~'^[0-9]{4}-[0-9]{2}-[0-9]{2}$' OR command->>'timezone'<>'Asia/Manila'
 THEN RETURN jsonb_build_object('success',false,'code','VALIDATION_REJECTED');END IF;
 BEGIN target_rental_id=command->>'rentalId';target_deur_id=command->>'deurId';target_work_date=(command->>'workDate')::date;
 ...
 IF NOT EXISTS(SELECT 1 FROM erp.rentals r JOIN erp.rental_equipment_lines l ON l.rental_id=r.id AND l.company_id=r.company_id AND l.deleted_at IS NULL
   ...
   WHERE r.id=target_rental_id AND d.id=target_deur_id AND r.company_id='TENANT-LOCAL-001' AND r.status='Active'
     AND coalesce(nullif(l.operational_metadata#>>'{deurExpectationSnapshot,policy,timezone}',''),nullif(r.timezone,''))='Asia/Manila'
