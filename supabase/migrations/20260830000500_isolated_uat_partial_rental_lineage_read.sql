...
BEGIN
 ...
 SELECT * INTO residue FROM erp.uat_multi_equipment_provisioning_scenarios WHERE company_id=tenant AND scenario_key=scenario_key;
 IF residue.scenario IS NULL OR residue.scenario->>'rentalAId'<>rental_id THEN RETURN jsonb_build_object('success',false,'status','FORBIDDEN'); END IF;
