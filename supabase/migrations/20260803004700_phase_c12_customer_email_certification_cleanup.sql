...
BEGIN
  ...
  IF TG_OP='DELETE' AND session_user=database_owner AND current_user=database_owner AND (
    ...
    (current_setting('erp.c12_customer_email_fixture_cleanup',true)='TENANT-UAT-C12-CUSTOMER-EMAIL-001' AND OLD.company_id='TENANT-UAT-C12-CUSTOMER-EMAIL-001')
...
BEGIN
  ...
  IF TG_OP='DELETE' AND TG_TABLE_SCHEMA='erp' AND TG_TABLE_NAME IN('commercial_snapshots','deur_activity_logs','deur_review_history','equipment_history','audit_log','recovery_compensations')
    AND session_user=database_owner AND current_user=database_owner AND (
      ...
      (current_setting('erp.c12_customer_email_fixture_cleanup',true)='TENANT-UAT-C12-CUSTOMER-EMAIL-001' AND target_company='TENANT-UAT-C12-CUSTOMER-EMAIL-001')
    ) THEN RETURN OLD; END IF;
  RAISE EXCEPTION 'immutable historical record cannot be changed' USING ERRCODE='55000';
END $$;
...
BEGIN
  ...
  IF TG_OP = 'DELETE'
     ...
     AND (
       ...
       OR
       (current_setting('erp.c12_customer_email_fixture_cleanup', true) = 'TENANT-UAT-C12-CUSTOMER-EMAIL-001'
         AND OLD.company_id = 'TENANT-UAT-C12-CUSTOMER-EMAIL-001')
     ) THEN
    RETURN OLD;
  END IF;
...
DECLARE database_owner name; BEGIN SELECT pg_get_userbyid(datdba) INTO database_owner FROM pg_database WHERE datname=current_database();
 IF TG_OP='DELETE' AND session_user=database_owner AND current_user=database_owner AND ((current_setting('erp.c7_release_fixture_cleanup',true)='TENANT-UAT-C7-RELEASE-001' AND OLD.company_id='TENANT-UAT-C7-RELEASE-001') OR (current_setting('erp.c7_normalization_fixture_cleanup',true)='TENANT-UAT-C7-NORMALIZE-001' AND OLD.company_id='TENANT-UAT-C7-NORMALIZE-001') OR (current_setting('erp.c12_customer_email_fixture_cleanup',true)='TENANT-UAT-C12-CUSTOMER-EMAIL-001' AND OLD.company_id='TENANT-UAT-C12-CUSTOMER-EMAIL-001')) THEN RETURN OLD; END IF;
...
CREATE FUNCTION erp.cleanup_c12_customer_email_certification_fixture(target_tenant_id text,expected_tenant_code text,confirmation text)
...
REVOKE ALL ON FUNCTION erp.protect_deur_event_history(),erp.reject_immutable_change(),erp.reject_customer_review_evidence_change(),erp.reject_terminal_notification_change(),erp.cleanup_c12_customer_email_certification_fixture(text,text,text) FROM PUBLIC,anon,authenticated,service_role;
COMMENT ON FUNCTION erp.cleanup_c12_customer_email_certification_fixture(text,text,text) IS 'Owner-only exact cleanup for disposable TENANT-UAT-C12-CUSTOMER-EMAIL-001 certification data; aggregate counts only.';
COMMIT;
