...
BEGIN
  ...
  IF (SELECT count(*) FROM companies WHERE id=target_tenant_id)>1
    ...
    OR (SELECT count(*) FROM operators WHERE company_id=target_tenant_id)>3
    ...
    OR (SELECT count(*) FROM equipment WHERE company_id=target_tenant_id)>3
    OR (SELECT count(*) FROM assignments WHERE company_id=target_tenant_id)>3
    OR (SELECT count(*) FROM rentals WHERE company_id=target_tenant_id)>1
    OR (SELECT count(*) FROM rental_equipment_lines WHERE company_id=target_tenant_id)>3
    ...
    OR (SELECT count(*) FROM deurs WHERE company_id=target_tenant_id)>3
    OR (SELECT count(*) FROM customer_review_requests WHERE company_id=target_tenant_id)>2
    ...
    OR (SELECT count(*) FROM customer_review_batch_items WHERE company_id=target_tenant_id)>6
...
COMMENT ON FUNCTION erp.cleanup_c12_grouped_customer_review_fixture(text,text,text) IS
  'Owner-only exact cleanup for the three-line TENANT-UAT-C12-GROUPED-CUSTOMER-001 certification; supports two Submitted lines, one In-Progress line, two daily batches, six items, and no notification/provider evidence; auth.users excluded.';
COMMIT;
