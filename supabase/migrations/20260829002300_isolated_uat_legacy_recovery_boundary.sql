...
DECLARE
  ...
  scenario_key text=trim(command->>'scenarioKey'); profile text=trim(command->>'profileVersion');
...
BEGIN
  IF auth.role()<>'service_role' OR jsonb_typeof(command)<>'object'
    ...
    OR tenant<>'TENANT-LOCAL-001' OR scenario_key<>'MULTI-EQUIPMENT-RUNTIME-CERT-2026-08-29'
  ...
  IF NOT pg_try_advisory_xact_lock(hashtextextended(tenant||':uat-multi-equipment:'||scenario_key,0))
  ...
  SELECT * INTO residue FROM erp.uat_multi_equipment_provisioning_scenarios
    WHERE company_id=tenant AND scenario_key=scenario_key FOR UPDATE;
  ...
  SELECT count(*) INTO attempts FROM erp.uat_multi_equipment_execution_attempts a WHERE a.company_id=tenant AND a.scenario_key=scenario_key;
  IF attempts<>0 OR EXISTS(SELECT 1 FROM erp.uat_multi_equipment_execution_attempts a WHERE a.company_id=tenant AND a.scenario_key=scenario_key AND a.status='RUNNING' AND a.lease_expires_at>clock_timestamp())
  ...
  SELECT EXISTS(SELECT 1 FROM erp.audit_log WHERE company_id=tenant AND aggregate_type='UatSyntheticScenario' AND aggregate_id=scenario_key AND action='UAT_LEGACY_PROVISIONING_RECOVERED') INTO audit_exists;
  UPDATE erp.uat_multi_equipment_provisioning_scenarios SET state='FAILED',updated_at=clock_timestamp() WHERE company_id=tenant AND scenario_key=scenario_key AND state='PROVISIONING';
  IF NOT audit_exists THEN
    ...
    VALUES(extensions.gen_random_uuid()::text,tenant,'UatSyntheticScenario',scenario_key,'UAT_LEGACY_PROVISIONING_RECOVERED',actor::text,clock_timestamp(),jsonb_build_object('previousState','PROVISIONING','newState','FAILED'),jsonb_build_object('reason','stale legacy residue after zero-core-artifact and zero-downstream-lineage guard'));
  END IF;
  RETURN jsonb_build_object('success',true,'disposition','RECOVERED','state','FAILED','scenarioKey',scenario_key);
END $$;
