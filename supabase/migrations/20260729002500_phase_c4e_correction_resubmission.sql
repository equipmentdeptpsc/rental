...
BEGIN
  scope=validate_deur_command_scope(command,'deur.review');
  IF scope->>'code'<>'OK' THEN RETURN jsonb_build_object('success',false,'code',scope->>'code'); END IF;
  idem=begin_deur_command(command,'SUBMIT_DEUR');
  ...
  SELECT d.* INTO current_deur FROM erp.deurs AS d
    WHERE d.id=(command->>'deurId') AND d.company_id=current_company_id() FOR UPDATE;
